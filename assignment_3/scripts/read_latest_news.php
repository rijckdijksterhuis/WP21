<?php if (isset($_POST['call_now'])){
    ...
    foreach($articles as $key => $value) {
        ...
        $articles_html .= sprintf('<button class="btn btn-danger article_remove" data-article-id="%s">Remove</button>', $value['id']);
        $articles_html .= '</div>';
        $articles_html .= '</div>';
        $articles_html .= '</div>';
    ...
    $export_data = [ 'html' => $articles_html ];
